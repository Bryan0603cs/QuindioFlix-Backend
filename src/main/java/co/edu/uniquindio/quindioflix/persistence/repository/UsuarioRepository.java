...
public interface UsuarioRepository extends JpaRepository<UsuarioEntity, Long> {
    ...
    @Query("""
            ...
            SET u.estadoCuenta = :nuevoEstado
            WHERE u.fechaVencimiento IS NOT NULL
              AND u.fechaVencimiento < :limite
              AND u.estadoCuenta = :estadoActual
            """)
    int desactivarCuentasVencidas(
            @Param("limite") LocalDateTime limite,
            @Param("estadoActual") EstadoCuenta estadoActual,
            @Param("nuevoEstado") EstadoCuenta nuevoEstado
    );
}
